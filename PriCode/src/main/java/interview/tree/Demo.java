...
public class Demo {
     ...
     */
    public static void getSumPath2(TreeNode root, ArrayDeque<TreeNode> ad, int sum, int currentSum){
        if(root == null){
            return;
        }
        currentSum += root.val;
        ...
        //判断是否为叶节点，如果是叶节点，判断其和是否与sum相同，如果相同则输出
        boolean hasNoChild = (root.left == null) && (root.right == null);
        if(currentSum == sum && hasNoChild){
            System.out.println(ad);
        }

        //如果不是叶节点，遍历他的左右节点
        getSumPath2(root.left, ad, sum, currentSum);
        getSumPath2(root.right, ad, sum, currentSum);

        //在返回父节点之前，删除当前节点
        ad.pop();
    }

    /** 等于sum的时候当前node不一定在leaf，且当前node=sum后不需要再往下遍历 **/
    public static void getSumPath_beta2(TreeNode root, ArrayDeque<TreeNode> ad, int sum, int currentSum){
        if(root == null) { return; }
        currentSum += root.val;
        ad.push(root);
        ...
        if(currentSum == sum) {
            ...
            getSumPath_beta2(root.left, ad, sum, currentSum);
            getSumPath_beta2(root.right, ad, sum, currentSum);
        }
        ad.pop();
    }
